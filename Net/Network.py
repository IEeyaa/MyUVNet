...
# 定义了相关的神经网络
import torch
from dgl.nn.pytorch import NNConv, MaxPooling
...
class CNNFace(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        # Conv: Norm, LeakyReLU
        self.conv1 = _conv2d(input_channels, 64, 3, padding=1, bias=False)
        self.conv2 = _conv2d(in_channels=64, out_channels=128, kernel_size=3)
        self.conv3 = _conv2d(in_channels=128, out_channels=256, kernel_size=3)

        # Poo: AVG
        self.final_pool = nn.AdaptiveAvgPool2d(1)

        # FC: Norm, LeakyReLU
        self.fc = _fc(256, output_channels)
...
class CNNEdge(nn.Module):
    def __init__(self, input_channels, output_channels):
        super(CNNEdge, self).__init__()

        # Conv: Norm, LeakyReLU
        self.conv1 = _conv1d(in_channels=input_channels, out_channels=64, kernel_size=3)
        self.conv2 = _conv1d(in_channels=64, out_channels=128, kernel_size=3)
        self.conv3 = _conv1d(in_channels=128, out_channels=256, kernel_size=3)
